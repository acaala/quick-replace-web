use std::{fs, sync::{Arc, atomic::{AtomicUsize, Ordering}}, collections::HashMap, borrow::Cow, process, path::Path};
...
async fn main() {
    ...
    // TODO: 404.
    let index = warp::path::end().map(|| warp::reply::html(fs::read_to_string("./static/index.html").unwrap()));
...
async fn user_connected(ws: WebSocket, users: Users) {
    ...
    tokio::task::spawn(async move {
        ...
        while let Some(message) = rx.next().await {
            ...
            user_tx.send(msg).unwrap_or_else(|e| {
